...
export class LikesService {
  ...
  async add(
    ...
    const genres = (
      await Promise.all(
        artists.map(({ id }) => this.findGenresByArtist(id, this.headers)),
      )
    ...
    const userById: User = await this.userService.findUserById(
      userId.split(':')[1],
    )
  ...
  async findGenresByArtist(artistId: string, headers: any) {
    const { data } = await this.httpService
      .get<any>(`https://api.spotify.com/v1/artists/${artistId}`, { headers })
      .toPromise()
    return data
  }
  ...
  async findSongInApi(songId: string) {
    const { data } = await this.httpService
      .get<any>(`https://api.spotify.com/v1/tracks/${songId}`, {
  ...
  async deleteLike(like: LikeDTO, token: string) {
    ...
    const likedSong = await this.likeRepository.findOne({
      where: { spotifyId: like.id },
    })
